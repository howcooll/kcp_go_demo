...
public:
	...
	void set_event_callback(const client_event_callback_t& event_callback_func,
			void* var);
	...
	int connect(int udp_port_bind, const std::string& server_ip,
			const int server_port);

	// Following 2 functions are for async connection
	void set_connect_event_callback(
			const client_event_callback_t& event_callback_func, void* var) {
		pconnect_event_func_ = event_callback_func, connect_event_func_var_ =
				var;
	}
	...
	int connect_async(int udp_port_bind, const std::string& server_ip,
			const int server_port);
...
private:
	static void client_event_callback_func(kcp_conv_t conv,
			eEventType event_type, const std::string& msg, void* var);
	void handle_client_event_callback(kcp_conv_t conv, eEventType event_type,
