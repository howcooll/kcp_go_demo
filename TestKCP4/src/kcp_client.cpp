...
kcp_client::kcp_client(void) :
		connect_succeed_(false), pevent_func_(NULL), event_callback_var_(
		NULL), udp_port_bind_(0), server_port_(0), udp_socket_(-1), p_kcp_(
		NULL) {
...
void kcp_client::init_kcp(kcp_conv_t conv) {
	...
	ikcp_nodelay(p_kcp_, 0, 10, 0, 1); // 设置成1次ACK跨越直接重传, 这样反应速度会更快. 内部时钟5毫秒.}
}
int kcp_client::connect_async(int udp_port_bind, const std::string& server_ip,
	...
	(*pevent_func_)(p_kcp_->conv, eConnect, "", event_callback_var_);
	return 0;
}
...
int kcp_client::init_udp_connect(void) {
	...
	// udp connect
	{
		int ret = ::connect(udp_socket_, (const struct sockaddr*) (&servaddr_),
				sizeof(servaddr_));
		if (ret < 0) {
...
void kcp_client::do_recv_udp_packet_in_loop(void) {
	...
	if (ret_recv < 0) {
		...
		(*pevent_func_)(p_kcp_->conv, eDisconnect, err_detail,
				event_callback_var_);
		return;
	}
	...
	while (true) {
		...
		if (msg.size() > 0) {
			...
			if (pevent_func_ != NULL) {
				(*pevent_func_)(p_kcp_->conv, eRcvMsg, msg,
						event_callback_var_);
			}
			continue;
		}
